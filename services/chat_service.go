...
func GetAIStream(ctx context.Context, message string) <-chan StreamChunk {
	...
	go func() {
		...
		// 处理流创建错误
		if err != nil {
			fmt.Printf("[AI流] 创建流失败: %v\n", err)
			ch <- StreamChunk{Error: err}
			return
		}
		// 确保在函数返回时关闭流
		defer stream.Close()
		fmt.Println("[AI流] 流创建成功，开始接收数据")
		...
		for {
			...
			// 检查是否到达流末尾
			if err == io.EOF {
				fmt.Println("[AI流] 数据接收完成")
				return
			}
			// 处理接收错误
			if err != nil {
				fmt.Printf("[AI流] 接收数据错误: %v\n", err)
				ch <- StreamChunk{Error: err}
				return
			}
